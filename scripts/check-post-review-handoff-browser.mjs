...
const artifactsDir = path.join(root, 'artifacts', 'm31a');
const distDir = path.join(root, 'dist-m31a');
...
if (build.status !== 0) throw new Error(`M31-A browser proof build failed:\n${build.stdout}\n${build.stderr}`);
...
try {
  ...
  const profileDir = path.join(root, '.tmp-m31a-chrome');
  ...
  await navigate(cdp, `${baseUrl}/?reviewComplete=1`);
  await waitForText(cdp, 'Revisões em dia');
  await waitForText(cdp, 'Continuar missão');

  const handoff = await primaryAction(cdp, 'Continuar missão');
  assert.equal(handoff.count, 1, 'post-review Home must expose one contextual Continue mission CTA');
  assert.equal(handoff.href, baseline.href, 'post-review CTA must preserve the canonical Journey Engine destination');
  ...
  assert.ok(scrollWidth <= width + 1, 'post-review Home must not overflow horizontally');
  await capture(cdp, '02-post-review-continue-mission');

  await writeFile(path.join(artifactsDir, 'post-review-handoff.json'), `${JSON.stringify({
    baselineCta: 'Continuar sílabas',
    postReviewCta: 'Continuar missão',
    canonicalPath: handoff.href,
    viewport: '390x844',
  }, null, 2)}\n`);

  console.log('Lexia M31-A Post Review Handoff Browser: PASS (normal curriculum CTA → review complete → Continue missão, canonical path preserved)');
} finally {
  cdp?.close();
  chrome?.kill('SIGTERM');
