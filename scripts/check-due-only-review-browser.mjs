...
const artifactsDir = path.join(root, 'artifacts', 'm23');
...
try {
  ...
  const profileDir = path.join(root, '.tmp-m23-chrome');
  ...
  for (const viewport of viewports) {
    ...
    const evidenceName = `${viewport.name}-review-complete`;
    await navigate(cdp, `${baseUrl}/?reviewComplete=1`);
    await waitForText(cdp, 'Revisões em dia');
    await waitForText(cdp, 'Você terminou tudo que estava pronto para hoje.');
    const reviewCompleteParam = await cdp.evaluate('new URLSearchParams(location.search).get("reviewComplete")');
    assert.equal(reviewCompleteParam, '1', `${evidenceName}: completion handoff must remain explicit in URL`);
    const body = await cdp.evaluate('document.body?.innerText || ""');
    assert.ok(!body.includes('Não consegui atualizar a revisão'), `${evidenceName}: completion must not look like an error state`);
    assertSurface(await metrics(cdp), evidenceName);
    await capture(cdp, evidenceName);
  }

  console.log('Lexia Due-Only Review Browser M23: PASS (review completion × mobile-short/mobile/desktop = 3 screenshots; combined release evidence = 84)');
} finally {
  cdp?.close();
  chrome?.kill('SIGTERM');
