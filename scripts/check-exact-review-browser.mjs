...
const artifactsDir = path.join(root, 'artifacts', 'm21');
...
try {
  ...
  const profileDir = path.join(root, '.tmp-m21-chrome');
  ...
  for (const viewport of viewports) {
    ...
    assert.equal(targetParam, 'Z', `${evidenceName}: exact target must remain in URL`);
    assert.ok(!body.includes('Missão atual'), `${evidenceName}: campaign mission must not override review`);
    assert.ok(!body.includes('Missão recomendada'), `${evidenceName}: campaign recommendation must stay hidden in review`);
    assert.ok(!body.includes('Desenhe a letra I!'), `${evidenceName}: legacy default letter must not replace target Z`);
    assertSurface(await metrics(cdp), evidenceName);
    await capture(cdp, evidenceName);
  }

  console.log('Lexia Exact Review Browser M21: PASS (reviewTarget=Z preserved × mobile-short/mobile/desktop = 3 screenshots; combined release evidence = 69)');
} finally {
  cdp?.close();
  chrome?.kill('SIGTERM');
