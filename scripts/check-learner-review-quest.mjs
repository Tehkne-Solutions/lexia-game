...
assert.equal(mastered.nextPath, '/play?review=1&reviewTarget=A');
assert.equal(mastered.nextEntityKey, 'A');
assert.equal(mastered.chapters.find((chapter) => chapter.id === 'letters').dueCount, 1);
assert.equal(mastered.chapters.find((chapter) => chapter.id === 'letters').reviewPath, '/play?review=1&reviewTarget=A');
