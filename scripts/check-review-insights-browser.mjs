...
const artifactsDir = path.join(root, 'artifacts', 'm19');
...
try {
  ...
  const profileDir = path.join(root, '.tmp-m19-chrome');
  ...
  for (const viewport of viewports) {
    ...
    const evidenceName = `${viewport.name}-parent-review-insights`;
    await navigate(cdp, `${baseUrl}/parent`);
    try {
      await waitForText(cdp, 'Ritmo de Revisão');
      await waitForText(cdp, 'Prontas agora');
      await waitForText(cdp, 'Estabilidade média');
      await waitForText(cdp, 'Próxima revisão futura');
      const scrolled = await cdp.evaluate(`(() => {
        const heading = Array.from(document.querySelectorAll('h1,h2,h3,div')).find((node) => node.textContent?.trim() === 'Ritmo de Revisão');
        if (!heading) return false;
        heading.scrollIntoView({ block: 'start' });
        window.scrollBy(0, -80);
        return true;
      })()`);
      assert.equal(scrolled, true, `${evidenceName}: review card must be discoverable`);
      await sleep(350);
  ...
  console.log('Lexia Journey Review Insights Browser M19: PASS (Parent review readiness × mobile-short/mobile/desktop = 3 screenshots; combined release evidence = 57)');
} finally {
  cdp?.close();
  chrome?.kill('SIGTERM');
